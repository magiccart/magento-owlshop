...
//jQuery(function($){
	function getDirMcolorpicker () {
			...
			dir = detected!==false ? detected : 'magiccart/';
		return dir;
	}
	 ...
	 function detectDirMcolorpicker() {
		...
		for(var i=0; i<e.length; i+=1) {
			if(e[i].src && /(^|\/)mcolorpicker.hack\.js([?#].*)?$/i.test(e[i].src)) {
	...
	jQuery.fn.mColorPicker.defaults.imageFolder = $images;
...
// Add by DVN
jQuery( document ).ready(function($) {
	$(".color").attr("data-hex", true).width("250px").mColorPicker(); //$(".color").mColorPicker();
