 ...
 * @@Modify Date: 2015-12-04 15:07:40
 * @@Function:
 */
...
jQuery(document).ready(function($) {
	...
	if(parseInt(switcher)<0) switcher = 767;
	var moblieContainer = ".header-right";//'.magicmenu';
	var desktopContainer = ".magicmenu";//'.magicmenu';
	//var hasMenuMobile = ".header-container";
	var menuDesktop = $(desktopContainer);
	var cloneMenuMobile  = $(moblieContainer);
	if($(window).width() <= switcher){
		menuDesktop.hide();
		cloneMenuMobile.parent().addClass('mode-mobile');
		//hasMenuMobile.addClass('hasmobile');
	}
	$(window).resize(function(){
		...
		if($window <= switcher){
			...
			menuDesktop.show();
			cloneMenuMobile.parent().removeClass('mode-mobile');
			//hasMenuMobile.removeClass('hasmobile');
		}

 	})	
    menuMobile.meanmenu({
    	meanMenuContainer: moblieContainer,
	...
	(function(selector){
	    ...
	    if($navDesktop.hasClass('sticker')){			
			$(window).scroll(function () {
			 if ($(this).scrollTop() > 700) {
			  ...
			  $('.header-sticker').removeClass('custom-header');
			 } 
			 else{
			  $('.header-sticker').removeClass('header-container-fixed');
			  $('.header-sticker').addClass('custom-header');
			 }
			 return false;
			});
	    }
		...
		var $navtop = $content.find('li.level0').not('.dropdown, .home, .ext');
		...
		function setReponsive($window){
			if (767 <= $window){
				...
				var $navtop = $content.find('li.level0').not('.dropdown, hasChild, .home, .ext');
