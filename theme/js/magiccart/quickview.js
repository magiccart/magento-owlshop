jQuery(function($) {
	...
	function _qsJnit(){
		...
		qsHandlerImg.fancybox({
				...
				beforeLoad : function(){ spConfig = false; isQuickview = true},
				beforeShow : function(){
			        ...
			        //btnCart.magiccart({url: getMagicUrl('magicshop/ajax')}); // comment in 1.9.10
					//quickview.find('.cloud-zoom, .cloud-zoom-gallery').CloudZoom({position: 'inside'}); //CloudZoom( {position: 'inside'});
			        var $thumbslide = $(".galleryimages");
			        var $typeslide  = $thumbslide.data('slide');
			        //if($typeslide){
			            var isVertical = false;
			            //if($typeslide == 'vertical') isVertical = true;
			            quickview.find(".product-image-thumbs").flexisel({clone: true, vertical: isVertical,visibleItems: 3});
			       // }
