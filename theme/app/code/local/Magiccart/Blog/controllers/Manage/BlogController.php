...
class Magiccart_Blog_Manage_BlogController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function indexAction()
    {
        ...
        $this
            ->_initAction()
            ->_addContent($this->getLayout()->createBlock('blog/manage_blog'))
            ->renderLayout()
        ;
    }
        ...
        if ($data = $this->getRequest()->getPost()) {
            ...
            if(isset($_FILES['image']['name']) && $_FILES['image']['name'] != '') {
                try {   
                    ...
                    $path = Mage::getBaseDir('media') . DS . 'magiccart' . DS .'blog'. DS;
                    $result = $uploader->save($path, $_FILES['image']['name'] );
                    
                    //this way the name is saved in DB
                    $data['image'] = 'magiccart/blog/'. $result['file'];
                } catch (Exception $e) {
